...
def transform_image(image, code_vectors):
    ...
    dist = np.linalg.norm(image.reshape(N*D, image.shape[2]) - np.expand_dims(code_vectors, axis=1), axis=2)
    new_im = code_vectors[np.argmin(dist, axis=0)].reshape(N, D, image.shape[2])
